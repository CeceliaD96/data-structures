...
public class NewSortedList implements List
{
	...
	{ 
		...
		if(prevElement!=null)
		{
			if(element.compareToIgnoreCase(prevElement.getInfo())>0)
				next = prevElement; //points to the last node inserted
			else
				next = list;
		}
		else
			next = list; // Points to the beginning of the list and starts there
